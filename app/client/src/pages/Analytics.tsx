  ...
  if (!setup || !overview) {
    return (
      <div className="text-center py-16 space-y-3">
        <BarChart3 size={48} className="mx-auto text-text-muted" />
        <h3 className="text-lg font-semibold text-text">No analytics data</h3>
        <p className="text-sm text-text-muted">Could not load analytics for this project.</p>
      </div>
    );
  }
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            disabled={refreshing}
            className="p-1.5 rounded-md text-text-muted hover:text-text transition-colors disabled:opacity-50"
            title="Refresh analytics"
          >
            <RefreshCw size={14} className={refreshing ? 'animate-spin' : ''} />
          </button>
          <div className="flex gap-1 bg-surface rounded-lg p-1 border border-line">
...
function StatCard({
  ...
  return (
    <div className="bg-surface border border-line rounded-xl p-5 space-y-2">
      ...
      <div className="text-[11px] text-text-muted">{period}</div>
    </div>
  );
}
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-divider">
            <th className="text-left px-5 py-2 text-xs font-medium text-text-muted">{headers[0]}</th>
            <th className="text-right px-5 py-2 text-xs font-medium text-text-muted">{headers[1]}</th>
          </tr>
        </thead>
        <tbody>
          {rows.map((row, i) => (
            <tr
              key={row[0]}
              className={i % 2 === 0 ? 'bg-surface' : 'bg-page'}
            >
              <td className="px-5 py-2.5 text-text-soft truncate max-w-[300px]">{row[0]}</td>
              <td className="px-5 py-2.5 text-text text-right font-medium tabular-nums">{row[1]}</td>
            </tr>
          ))}
        </tbody>
      </table>
    ...
    <div className="space-y-6">
      ...
      <div className="bg-surface border border-line rounded-xl p-6 space-y-3">
        <div className="flex items-center gap-2.5">
          ...
          <h3 className="text-lg font-semibold text-text">Add Analytics to Your App</h3>
        </div>
      ...
      <div className="bg-surface border border-line rounded-xl p-6 space-y-6">
        ...
        <SetupStep number={1} title="Copy the script tag">
          <p className="text-sm text-text-soft leading-relaxed">
            Click the <strong className="text-text">copy button</strong> on the right side of the code
        ...
        <SetupStep number={2} title="Paste it into your HTML">
          <p className="text-sm text-text-soft leading-relaxed">
            ...
            <code className="px-1.5 py-0.5 rounded bg-page text-brand text-xs font-mono">&lt;head&gt;</code>{' '}
            section — right before the closing{' '}
            <code className="px-1.5 py-0.5 rounded bg-page text-brand text-xs font-mono">&lt;/head&gt;</code>{' '}
            tag.
          </p>
          <div className="bg-page rounded-lg p-4 border border-divider text-xs font-mono text-text-soft leading-relaxed">
            <span className="text-text-muted">&lt;head&gt;</span><br />
            <span className="text-text-muted pl-4">  &lt;!-- your other tags ... --&gt;</span><br />
            <span className="pl-4 text-success">{setup.scriptTag}</span>
            <span className="text-success/60">  ← paste here</span><br />
            <span className="text-text-muted">&lt;/head&gt;</span>
          </div>
          <details className="mt-2 text-sm">
            <summary className="text-text-muted hover:text-text cursor-pointer flex items-center gap-1.5">
              <FileCode2 size={13} />
              Where is my HTML file?
            </summary>
            <ul className="mt-2 space-y-1.5 text-text-soft text-xs leading-relaxed pl-5 list-disc">
              <li><strong className="text-text">React</strong> (Vite / Create React App) → <code className="px-1 py-0.5 rounded bg-page text-brand font-mono">index.html</code> in the project root or <code className="px-1 py-0.5 rounded bg-page text-brand font-mono">public/</code> folder</li>
              <li><strong className="text-text">Next.js</strong> → <code className="px-1 py-0.5 rounded bg-page text-brand font-mono">app/layout.tsx</code> (inside the <code className="font-mono text-brand">&lt;head&gt;</code>) or use <code className="font-mono text-brand">next/script</code></li>
              <li><strong className="text-text">Plain HTML</strong> → your main <code className="px-1 py-0.5 rounded bg-page text-brand font-mono">.html</code> file</li>
              <li><strong className="text-text">WordPress</strong> → Appearance → Theme Editor → <code className="px-1 py-0.5 rounded bg-page text-brand font-mono">header.php</code></li>
              <li><strong className="text-text">Not sure?</strong> Ask your AI coding assistant: "Where should I put a script tag in my app?"</li>
            </ul>
          </details>
        </SetupStep>
        ...
        <div className="space-y-2">
          <h5 className="text-xs font-semibold text-text uppercase tracking-wider">Optional — Track custom events</h5>
          <p className="text-sm text-text-soft leading-relaxed">
          ...
          <div className="bg-page rounded-lg p-4 border border-divider text-xs font-mono text-text-soft leading-relaxed space-y-1">
            <div><span className="text-text-muted">// Example: track a button click</span></div>
            <div>
              <span className="text-text">takeoff</span>
              <span className="text-text-muted">.</span>
              <span className="text-text">track</span>
              <span className="text-text-muted">(</span>
              <span className="text-success">'signup_clicked'</span>
              <span className="text-text-muted">);</span>
            </div>
            <div className="pt-2"><span className="text-text-muted">// You can also attach extra info</span></div>
            <div>
              <span className="text-text">takeoff</span>
              <span className="text-text-muted">.</span>
              <span className="text-text">track</span>
              <span className="text-text-muted">(</span>
              <span className="text-success">'purchase'</span>
              <span className="text-text-muted">, {'{'} </span>
              ...
              <span className="text-text-muted">: </span>
              <span className="text-text">29</span>
              <span className="text-text-muted"> {'}'});</span>
            </div>
          </div>
        </div>
      ...
      <div className="bg-surface border border-line rounded-xl p-5 space-y-4">
        <div className="flex items-center gap-2">
          ...
          <h4 className="text-sm font-medium text-text">Privacy-friendly by default</h4>
        </div>
        <ul className="text-sm text-text-soft space-y-1.5 list-disc pl-5 leading-relaxed">
...
function SetupStep({
  ...
  return (
    <div className="flex gap-4">
      <div className="flex-shrink-0 w-7 h-7 rounded-full bg-brand-tint border border-brand-tint-border flex items-center justify-center">
        <span className="text-xs font-bold text-brand">{number}</span>
      </div>
      <div className="flex-1 space-y-3 pt-0.5">
        <h5 className="text-sm font-medium text-text">{title}</h5>
        {children}
      </div>
    </div>
  );
...
function CodeSnippetBlock({
  ...
  return (
    <div className="space-y-2">
      {label && <label className="block text-xs font-medium text-text-muted">{label}</label>}
      <div className="relative">
        <pre className="text-xs text-text-soft bg-page rounded-lg p-4 pr-12 border border-divider overflow-auto font-mono">
          {code}
        </pre>
        <button
          onClick={onCopy}
          className="absolute top-2.5 right-2.5 p-1.5 rounded-md bg-surface-2 border border-line text-text-muted hover:text-text transition-colors"
...
function NpmAlternative({
  ...
  return (
    <div>
      ...
      {showNpm && (
        <div className="mt-3 space-y-3 pl-5 border-l-2 border-divider">
          <p className="text-xs text-text-soft leading-relaxed">
            If you prefer importing analytics as a dependency instead of a script tag,
            run this in your project folder:
          </p>
          <pre className="text-xs text-text-soft bg-page rounded-lg p-4 border border-divider overflow-auto font-mono">
            {setup.npmInstall}
          </pre>
          <p className="text-xs text-text-soft leading-relaxed">
            Then add this to your app's entry point (e.g.{' '}
            <code className="px-1 py-0.5 rounded bg-page text-brand font-mono">main.tsx</code> or{' '}
            <code className="px-1 py-0.5 rounded bg-page text-brand font-mono">App.jsx</code>):
          </p>
...
function FaqItem({ q, a }: { q: string; a: string }) {
  return (
    <div className="bg-page rounded-lg p-3 space-y-1">
      <p className="text-text font-medium text-xs">{q}</p>
      <p className="text-text-soft text-xs leading-relaxed">{a}</p>
    </div>
  );
}
