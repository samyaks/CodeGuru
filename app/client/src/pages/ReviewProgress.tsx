    ...
    <div className="min-h-screen">
      ...
      <main className="max-w-2xl mx-auto px-6 py-16">
        <div className="flex flex-col items-center space-y-8 text-center">
          {!failed && <Loader size={32} className="animate-spin text-brand" />}
          {failed && <AlertCircle size={32} className="text-danger" />}

          <div className="space-y-2">
            <h2 className="text-xl font-semibold">
              {failed ? 'Review failed' : 'Reviewing code'}
            </h2>
            <p className="text-text-muted text-sm">{currentMessage}</p>
          </div>
          ...
          <div className="w-full space-y-3 text-left">
            {steps.map((step, i) => (
              ...
              >
                {step.status === 'done' && <Check size={16} className="text-success shrink-0" />}
                {step.status === 'active' && <Loader size={16} className="animate-spin text-brand shrink-0" />}
                {step.status === 'error' && <AlertCircle size={16} className="text-danger shrink-0" />}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {step.status === 'active' && step.message && (
                    <p className="text-xs text-text-muted truncate mt-0.5">{step.message}</p>
                  )}
                </div>
              </div>
            ))}
          ...
          {sseError && (
            <p className="text-danger text-sm">Connection lost. Refresh to check status.</p>
          )}
        </div>
      </main>
    </div>
