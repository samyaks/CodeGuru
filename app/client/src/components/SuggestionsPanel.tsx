...
export default function SuggestionsPanel({ projectId, projectStatus }: { projectId: string; projectStatus: string }) {
    ...
    fetchSuggestions(projectId)
      .then((data) => {
        ...
        if (!hasAi && data.suggestions.length > 0 && projectStatus === 'ready') {
          const reFetch = () =>
            fetchSuggestions(projectId)
              .then((fresh) => { if (!cancelled) setSuggestions(fresh.suggestions); })
              .catch(() => {});
  ...
  const handleDismiss = async (suggestionId: string) => {
    ...
    try {
      ...
      setSuggestions((s) => s.map((x) => (x.id === suggestionId ? { ...x, status: 'open' as const } : x)));
    }
  };
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-16">
        <Loader2 size={24} className="animate-spin text-sky-muted" />
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      {/* AI polling banner */}
      {suggestions.length > 0 && !suggestions.some((s) => s.source === 'ai') && projectStatus === 'ready' && !aiDonePolling && (
      ...
      <div className="flex items-center gap-2 text-sm flex-wrap">
        <button
          ...
          className={`px-4 py-1.5 rounded-lg transition-colors ${
            activeFilter === 'all' ? 'bg-gold/15 text-gold border border-gold/30' : 'text-sky-muted hover:text-sky-white'
          }`}
        >
        ...
        {Object.entries(typeCounts).map(([type, count]) => (
          <button
            ...
            className={`px-4 py-1.5 rounded-lg transition-colors ${
              activeFilter === type ? 'bg-gold/15 text-gold border border-gold/30' : 'text-sky-muted hover:text-sky-white'
            }`}
          >
