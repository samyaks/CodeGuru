...
async function startReview(reviewId) {
  try {
    ...
    if (review.type === 'pr') {
      report = await runPRReview(reviewId, review);
    } else {
      report = await runRepoReview(reviewId, review);
    }
...
async function runPRReview(reviewId, review) {
  ...
  return reviewer.reviewPR(reviewId, {
    owner: review.owner,
    repo: review.repo,
...
async function runRepoReview(reviewId, review) {
  ...
  return reviewer.reviewRepo(reviewId, {
    owner: review.owner,
    repo: review.repo,
    files: fileContents,
    deployInfo,
