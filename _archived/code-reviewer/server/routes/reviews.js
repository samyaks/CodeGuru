...
const { reviews, reviewFiles } = require('../lib/db');
...
router.post('/', async (req, res) => {
  try {
    ...
    if (type === 'pr' && !prNumber) {
      ...
      if (!parsed) {
        return res.status(400).json({ error: 'prNumber is required for PR reviews, or provide a full PR URL' });
      }
...
async function createReview(res, { repoUrl, prNumber, type, owner, repo, branch }) {
  ...
  console.log(`Created ${type} review ${id} for ${owner}/${repo}${prNumber ? ` PR #${prNumber}` : ''}`);

  // Kick off the review process asynchronously (Phase 2 will implement the reviewer service)
...
async function startReview(reviewId) {
  try {
    ...
    if (review.type === 'pr') {
      result = await runPRReview(reviewId, review);
    } else {
      result = await runRepoReview(reviewId, review);
    }

    const { report, fileContents } = result;

    // Persist per-file AI comments and severities
    ...
    // Generate fix prompts asynchronously (don't block review completion)
    setImmediate(async () => {
      try {
...
async function runPRReview(reviewId, review) {
  broadcast(reviewId, { type: 'progress', phase: 'fetching', message: `Fetching PR #${review.pr_number} from ${review.owner}/${review.repo}...` });
  ...
  const fileRecords = prFiles.map((f) => ({
    id: uuidv4(),
    review_id: reviewId,
  ...
  // Build fileContents from PR patches for fix prompt convention search
  const fileContents = prFiles.map((f) => ({
    path: f.filename,
...
async function runRepoReview(reviewId, review) {
  broadcast(reviewId, { type: 'progress', phase: 'fetching', message: `Fetching repository tree for ${review.owner}/${review.repo}...` });

  // Resolve the actual default branch if none was specified
  ...
  try {
    ...
    // Fallback: try 'master' if 'main' failed
  ...
  broadcast(reviewId, { type: 'progress', phase: 'fetched', message: `Found ${codeFiles.length} files in repository`, fileCount: codeFiles.length });

  // Detect deployment configuration files
  const deployInfo = detectDeploymentFiles(tree);
  if (deployInfo.detected) {
    broadcast(reviewId, { type: 'progress', phase: 'fetching', message: `Found ${deployInfo.allPaths.length} deployment config files, fetching...` });
  }

  const MAX_DEPLOY_FILES = 10;
  const deployFilePaths = deployInfo.allPaths.slice(0, MAX_DEPLOY_FILES);
  const deployFileContents = await Promise.all(
    deployFilePaths.map((p) =>
      github.fetchFileContent(review.owner, review.repo, p, branch).catch(() => ({ path: p, content: null, size: 0 }))
    )
  );

  const MAX_FILES = 20;
  const sample = codeFiles.slice(0, MAX_FILES);
  ...
  const fileRecords = fileContents.map((f) => ({
    id: uuidv4(),
    review_id: reviewId,
    file_path: f.path,
    diff: null,
  ...
  const report = await reviewer.reviewRepo(reviewId, {
    ...
    files: fileContents,
    deployInfo,
    deployFiles: deployFileContents,
  });
...
router.get('/', (req, res) => {
  ...
  const list = reviews.list({ limit: parseInt(limit, 10), offset: parseInt(offset, 10) });
  res.json(list);
});

// Get a single review with its file-level data
...
// Update human notes on a review
...
// Update human comments on a specific file
