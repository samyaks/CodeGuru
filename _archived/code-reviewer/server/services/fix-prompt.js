...
/**
 * Generate fix prompts for all actionable findings in a completed review.
 */
async function generateFixPrompts(reviewId, review, report, fileContents) {
  ...
  console.log(`Generated ${prompts.length} fix prompts for review ${reviewId}`);
  return prompts;
}

/**
 * Call Claude to find convention references for all findings in one batch.
 */
async function findConventions(findings, fileContents) {
  ...
  try {
    ...
    try {
      ...
      console.error('Failed to parse convention search response');
      return [];
    }
  }
}
...
/**
 * Extract a code snippet from the fetched file contents for a finding.
 */
...
/**
 * Assemble the fix prompt text from the template.
 */
